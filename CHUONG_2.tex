...
\textbf{Ví dụ trong đề tài (Kaggle Dataset):} Dataset bao gồm Positive: 10,034 (43.3\%), Neutral: 9,083 (39.2\%), Negative: 4,072 (17.6\%), với $\text{IR} = 10,034 / 4,072 = 2.46$.
...
\subsubsection{Công thức Class Weighting}

Phương pháp đơn giản hơn Focal Loss: Tích hợp trọng số vào Cross Entropy.
...
với $N$ là tổng số mẫu (23,189), $K$ là số classes (3), và $n_j$ là số mẫu class $j$.

\textbf{Ví dụ tính toán (Kaggle dataset):}

\begin{align}
    w_{\text{Positive}} &= \frac{23,189}{3 \times 10,034} = 0.770 \\
    w_{\text{Neutral}} &= \frac{23,189}{3 \times 9,083} = 0.852 \\
    w_{\text{Negative}} &= \frac{23,189}{3 \times 4,072} = 1.898
\end{align}

$\rightarrow$ Negative class nhận trọng số \textbf{2.46$\times$} cao hơn Positive!
...
Trong biểu thức trên, $N$ biểu thị tổng số mẫu trong một batch, $y_i$ là nhãn thực tế, và $p_{y_i}$ là xác suất dự đoán của mô hình đối với lớp đúng. Thành phần quan trọng nhất là $w_{y_i}$, đại diện cho trọng số được gán riêng cho từng lớp, thường được tính dựa trên nghịch đảo tần suất xuất hiện của lớp đó trong tập dữ liệu. Cụ thể, các lớp thiểu số sẽ được gán trọng số lớn hơn ($w > 1$) và ngược lại đối với lớp đa số.
